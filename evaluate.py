...
def evaluate_segmentation(net, valid_iterator, device,n_valid_examples,is_avg_prec=False,prec_thresholds=[0.5,0.7,0.9],output_dir=None):
    ...
    with tqdm(total=n_valid_examples, desc='Segmentation Val round', unit='img') as pbar:
        for batch_idx,batch in enumerate(valid_iterator):
            ...
            with torch.no_grad():
                # predict the mask
                mask_pred, _ = net(images_device)  # Ignore second output (edges)
                ...
                if is_avg_prec:
                    ...
                    for i in range(true_masks_copy.shape[0]):
                        ...
                        if output_dir:
                            ...
                            cv2.imwrite(os.path.join(output_dir,'input_segmentation_images','images_{:04d}.tif'.format(batch_idx*true_masks_copy.shape[0]+i)),img)
                            cv2.imwrite(os.path.join(output_dir, 'segmentation_predictions', 'images_{:04d}.tif'.format(batch_idx*true_masks_copy.shape[0]+ i)),overlay_img)
                ...
                if is_avg_prec:
                    ...
                    for i in range(true_masks_copy.shape[0]):
                        ...
                        if output_dir:
                            ...
                            img=(img-np.min(img))/(np.max(img)-np.min(img))*255
                            overlay_img = visualize_segmentation(pred, inp_img=img, overlay_img=True)
                            cv2.imwrite(os.path.join(output_dir,'input_tracking_images','images_{:04d}.tif'.format(batch_idx*true_masks_copy.shape[0]+i)),img)
                            cv2.imwrite(os.path.join(output_dir, 'tracking_predictions', 'images_{:04d}.tif'.format(batch_idx*true_masks_copy.shape[0]+ i)),overlay_img)
