...
class SparseGraph:
    ...
    def __init__(self, cut_off_distance, allow_cell_division=True, density_map=None):
        """
        ...
        Args:
            cut_off_distance: distance threshold for minimum cost of appearance/deletion operations
            allow_cell_division: whether to allow cell division modeling
            density_map: optional density map for adaptive tracking
        """
        self.vertices = VerticesDict()
        self.edges = {}
        self.valid_edges = {
            'source': ['appear', 'normal'],
            'sink': [],
            'split': ['normal'],
        ...
        self.distance_threshold = self._adaptive_threshold(cut_off_distance)
        self.cell_division = allow_cell_division
        self.min_cell_area = 4
        self.split_cost_multiplier = 0.7 if allow_cell_division else 10

    def _adaptive_threshold(self, base_threshold):
        """Adjust thresholds based on local density"""
        if self.density_map is None:
            return base_threshold
        avg_density = np.mean(self.density_map)
        return base_threshold * (0.5 + 0.5*avg_density)

    def _get_local_density(self, position):
        """Get local density at given position"""
        if self.density_map is None:
            return 0
        try:
            ...
            x = max(0, min(x, self.density_map.shape[0]-1))
            y = max(0, min(y, self.density_map.shape[1]-1))
            return self.density_map[x, y]
        except (IndexError, TypeError):
            return 0
    def calc_vertex_split_cost(self, start_vertex, end_vertex):
        """Enhanced division detection with density awareness"""
        daughter_vertices = [v for v in end_vertex.out_edges.values() 
                           if v.end_vertex.type == 'normal']
        
        if len(daughter_vertices) != 2:
            return self.distance_threshold * 3
            
        # Enhanced division criteria
        d1_pos, d1_size = daughter_vertices[0].features[0], daughter_vertices[0].features[-1]
        d2_pos, d2_size = daughter_vertices[1].features[0], daughter_vertices[1].features[-1]
        mother_pos, mother_size = start_vertex.features[0], start_vertex.features[-1]
        
        # Dynamic thresholds based on local density
        local_density = self._get_local_density(mother_pos)
        size_ratio_threshold = 0.4 + 0.3 * local_density  # More permissive in dense areas
        distance_threshold = 2.5 * np.sqrt(mother_size) * (1 + local_density)
        
        # Calculate actual metrics
        size_ratio = min(d1_size, d2_size) / max(d1_size, d2_size)
        distance = compute_distance(d1_pos, d2_pos)
        total_size_ratio = (d1_size + d2_size) / mother_size
        
        # Enhanced decision making
        if (size_ratio > size_ratio_threshold and 
            0.7 < total_size_ratio < 1.4 and 
            distance < distance_threshold):
            return compute_distance(mother_pos, 0.5*(d1_pos+d2_pos)) * self.split_cost_multiplier
        return self.distance_threshold * 3
    ...
    def calc_vertex_split_cost(self, start_vertex, end_vertex):
        ...
        # Get features safely
        d1_size = daughter_vertices[0].features[-1] if len(daughter_vertices[0].features) > 1 else 1
        d2_size = daughter_vertices[1].features[-1] if len(daughter_vertices[1].features) > 1 else 1
        mother_size = start_vertex.features[-1] if len(start_vertex.features) > 1 else 1
        # Calculate ratios safely
        ratio_daughters = min(d1_size, d2_size) / max(d1_size, d2_size) if max(d1_size, d2_size) > 0 else 0
        ratio_d1_mother = d1_size / mother_size if mother_size > 0 else 10
        ratio_d2_mother = d2_size / mother_size if mother_size > 0 else 10
        # More permissive conditions for cell division
        if (ratio_daughters > 0.3 and
            (0.5 < (ratio_d1_mother + ratio_d2_mother) < 1.5 and
             dist_daughters < 3*np.sqrt(max(0.1, mother_size)))):
            return total_cost * 0.8  # Slightly favor divisions
        return self.distance_threshold*5
