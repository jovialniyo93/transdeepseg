...
class MultiCellTracker:
    """Tracks multiple cells in 2D+t and 3D+t using a coupled minimum-cost flow as matching strategy."""
    def __init__(self, config):
        self.config = config
        self.cell_rois = {}
        self.tracks = {}
        ...
        # Initialize sparse_tracking based on config
        self.sparse_tracking = self.config.seeds is not None
        
        # Density-aware attributes
        self.density_map = None
        self.cell_size_stats = {}
        self.adaptive_params = {
            'merge_threshold': 0.8,
            'split_threshold': 0.6,
            'max_skip_frames': 3 if getattr(self.config, 'tracking_strategy', None) == 'adaptive' else 2
        }
    ...
    def __call__(self):
        ...
        # Post-processing
        self.filter_short_tracks(min_frames=getattr(self.config, 'min_track_length', 5))
        return self.tracks

    def tracking_step(self, i, time):
        img = imread(self.config.get_image_file(time))
        
        # Build density map for adaptive tracking
        if i == 0:
            self._build_density_map(img)
            
        # Enhanced position estimation
        for track_id, cell_roi in self.cell_rois.items():
            cell_roi(time, img, self.density_map)
            
        if i > 0:
            segmented_candidates, all_masks = self.extract_candidates(time)
            # Density-aware matching
            self.match_objects(time, segmented_candidates, all_masks, img)

    def _build_density_map(self, img):
        """Create density map for adaptive parameter adjustment"""
        from scipy.ndimage import gaussian_filter
        binary = (img > 0).astype(float)
        self.density_map = gaussian_filter(binary, sigma=3)
        
    def match_objects(self, time, matching_candidates, all_masks, image):
        # Enhanced feature extraction with density weighting
        track_features = {}
        candidate_features = {}
        ...
        for track_id, candidates in matching_candidates.items():
            ...
            if candidates:
                # Density-aware feature weighting
                density_weight = self._get_density_weight(track.get_last_position())
                features_track = self._get_features(track, density_weight)
                features_candidates = self._get_candidate_features(candidates, density_weight)
                
                track_features[track_id] = features_track
                candidate_features[track_id] = features_candidates

        # Enhanced matching with adaptive thresholds
        matched_objects = graph_tracking(
            track_features, 
            candidate_features,
        ...
        self._validate_matches(matched_objects, time, matching_candidates, image)

    def filter_short_tracks(self, min_frames=5):
        """Remove tracks that are too short to be reliable"""
        valid_tracks = {}
        for track_id, track in self.tracks.items():
            if len(track.masks) >= min_frames:
    ...
    def __call__(self):
        ...
        for i, time in enumerate(time_steps):
            ...
            if i == 0:
                self.map_seeds_to_segmentation(time)
            self.tracking_step(i, time)
        self.filter_short_tracks(min_frames=5)
        return self.tracks
    ...
    """Optimized version for tracker.py _get_features method"""
    def _get_features(self, candidates, track):
        """ Extracts for each track its features and the features of its neighbors at the next step. """
        # Make sure all features have consistent format
        candidate_features = {}
        for m_id, positions in candidates.items():
            pos = np.median(positions, axis=1)
            area = len(positions[0])
            # Store position and area as a single 1D array
            candidate_features[m_id] = np.array([*pos, area])
        # Get track features in the same format
        estimated_track_position = self.cell_rois[track.track_id].last_roi().center
        area = len(track.masks[track.get_last_time()][0])
        # Combine position and area in the same array
        track_features = np.array([*estimated_track_position, area])
        return track_features, candidate_features
    ...
    def fill_in_dummy_masks(self, track_id, time, new_mask):
        """Creates dummy masks for missing objects."""
        prev_mask = self.tracks[track_id].masks[self.tracks[track_id].get_last_time()]
        #displacement = (np.median(new_mask, axis=1) - np.median(prev_mask, axis=1)).astype(int)
        displacement = (np.median(new_mask, axis=1) - np.median(prev_mask, axis=1)).astype(int)
        t_last = self.tracks[track_id].get_last_time()
        n_missing_steps = time - t_last
        for step in range(1, n_missing_steps):
    ...
    def map_seeds_to_segmentation(self, time):
        ...
        segmentation_mask, mask_indices = self.config.get_segmentation_masks(time)
        # Use connected components to better separate touching cells
        if len(segmentation_mask.shape) == 2:
            from skimage.measure import label
            labeled_mask = label(segmentation_mask)
            mask_indices = get_mask_positions(labeled_mask)
        # Lower area threshold for dense environments
        mask_indices = mask_indices[mask_indices.apply(lambda x: len(x[0]) >= 5)]
        ...
        if self.sparse_tracking:
            ...
            # More aggressive tracking of all objects
            for mask_id, coordinates in mask_indices.items():
                # Only init track if not already tracking this cell
                if not any(np.array_equal(coordinates, t.masks[time])
                           for t in self.tracks.values() if time in t.masks):
...
class TrackingConfig:
    ...
    def __init__(self, img_files, segm_files, seeds, roi_box_size, delta_t=2,
                 cut_off_distance=None, allow_cell_division=True,min_track_length=10,tracking_strategy='adaptive'):
        """
...
class ROI:
    ...
    def __init__(self, center_position, box_size):
        ...
        self.top_left = (np.array(center_position) - np.array(box_size) // 2).astype(int)
        self.bottom_right = (np.array(center_position) + np.array(box_size) // 2
                             + np.array(box_size) % 2).astype(int)
    ...
    def adapt_roi(self, center):
        ...
        self.top_left = (np.array(self.center) - np.array(self.box_size) // 2).astype(int)
        self.bottom_right = (np.array(self.center) + np.array(self.box_size) // 2
                           + np.array(self.box_size) % 2).astype(int)
