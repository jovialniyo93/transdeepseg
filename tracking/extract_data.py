...
'''def get_mask_positions(data, background_id=0):
    ...
    return df'''
def get_mask_positions(data, background_id=0):
    """More sensitive mask extraction for dense cell environments"""
    # Use more memory-efficient method for large arrays
    masked_data = (data != background_id)
    flat_data = data[masked_data]
    dummy_index = np.where(masked_data.ravel())[0]
    
    # Use pandas groupby for better handling of dense small objects
    df = pd.DataFrame({'mask_id': flat_data, 'flat_index': dummy_index})
    df = df.groupby('mask_id').apply(lambda x: np.unravel_index(x.flat_index.values, data.shape))
    
    # Filter out very small objects that might be noise
    df = df[df.apply(lambda x: len(x[0]) > 5)]  # Minimum 5 pixels
