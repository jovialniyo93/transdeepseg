...
def train(args,image_size = [576,576],image_means = [0.5],image_stds= [0.5],train_ratio = 0.85,save_checkpoint=True,if_train_aug=True,train_aug_iter=3,patience=5):
    ...
    train_data = BasicSegmentationDataset(os.path.join(args.train_set_dir, 'images'), os.path.join(args.train_set_dir, 'masks'), transforms=train_transforms, if_train_aug=if_train_aug, train_aug_iter=train_aug_iter)
    ...
    # Fixed deprecated GradScaler - use the new format
    ...
    for epoch in range(args.max_epoch):
        ...
        with tqdm(total=n_train_examples, desc=f'Epoch {epoch + 1}/{args.max_epoch}', unit='img') as pbar:
            for step,batch in enumerate(train_iterator):
                ...
                # Fixed deprecated autocast - use the new format
                with torch.amp.autocast('cuda', enabled=True):
                    # Model returns (logits, edges) but we only use logits for segmentation
                    masks_preds, _ = model(images)
        ...
        # Evaluation round - Fixed to unpack only 2 values
        # In the training loop, replace the evaluation part with:
        val_score, avg_precision = evaluate_segmentation(model, valid_iterator, device, n_valid_examples, is_avg_prec=((1+epoch)%2==0), prec_thresholds=[0.5])
        ...
        if avg_precision is not None and avg_precision[0] > avg_precision_best:
            avg_precision_best = avg_precision[0]
